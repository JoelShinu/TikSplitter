...
class AutoCaptioner:
    def __init__(self, output_path: Path = CAPTION_PATH):
        ...
        self._logger = configure_logging("captioner")
    ...
    def get_and_save_captions(self, video_url: str):
        try:
            ...
            if video_id:
                ...
                self._logger.error("Unable to extract video ID from the YouTube URL.")

        except Exception as e:
            self._logger.error("An error occurred while saving captions:")
    ...
    def get_captions(self, video_url: str) -> Iterable[Caption] | None:
        try:
            ...
            self._logger.error("An error occurred while fetching captions:")
            self._logger.exception(e)
            return None
    ...
    def extract_video_id(self, video_url: str) -> str | None:
        try:
            ...
            self._logger.error("An error occurred while extracting video ID:")
            self._logger.exception(e)
            return None
    ...
    def save_captions_as_srt(self, captions: Iterable[Caption], output_filename: str):
        try:
            ...
            self._logger.info(f"Captions saved as SRT file: {output_file_path}")

        except Exception as e:
            self._logger.error("An error occurred while saving captions as SRT:")
            self._logger.exception(e)
    ...
    def get_video_title(self, video_url: str) -> str:
        try:
            ...
            self._logger.error("An error occurred while fetching video title:")
            self._logger.exception(e)
            return ""
